...
void signal_handler(int i_Sig)
{
    ...
    string PIPED_NAME_STR = PIPED_NAME_1 + to_string(numOfMiners);
    const char* READ_PIPED_NAME = PIPED_NAME_STR.c_str();

    int Read_fd = open(READ_PIPED_NAME, O_RDONLY | O_NONBLOCK);
    ...
    ReadMinerFD.push_back(Read_fd);
    WriteMinerFD.push_back(Write_fd);

    write(Write_fd, &blockchain.back(), sizeof(Block));
...
void serverLoop() {
    ...
    while (true) {
        ...
        for(int i = 0; i< numOfMiners; i++)
        {
            ...
            read(ReadMinerFD[i], &testingBlock, sizeof(Block));
            if(testingBlock.m_Block.height != -1)
            {
